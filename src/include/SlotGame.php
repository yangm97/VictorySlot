...
function playFruitSlot() {
    // Play slots
    $slot_values = launchFruitSlot(); // Slot values ex. ['plum', 'bell', 'cherry']
    $score = getComboScore($slot_values); // Score for given value
    $indexes = getFruitIndexes($slot_values); // Indexes used by ezslots frontend
    // Format des données
    $result = array("values"=>$slot_values, "indexes"=>$indexes, "score"=>$score);
    return $result;
}

function launchFruitSlot() {
    $min_value = 1;
    $max_value = 20;

    // Generate some random stuff
    $reel_1 = getRandomVal($min_value, $max_value);
    $reel_2 = getRandomVal($min_value, $max_value);
    $reel_3 = getRandomVal($min_value, $max_value);

    $values = identifyValue(array($reel_1, $reel_2, $reel_3));
    return $values;
}
